...
DPFORAM::DPFORAM(const char *party, Connection *cons[2],
                 ...
                 uint log_n, uint data_bytes, bool is_last) : Protocol(party, cons, rnd, prgs) {
    ...
    if (this->is_first_) {
        ...
        this->InitMem(this->cache_[0]);
        this->InitMem(this->cache_[1]);
...
DPFORAM::~DPFORAM() {
    if (!this->is_first_) {
        ...
        DeleteMem(this->cache_[0]);
        DeleteMem(this->cache_[1]);
        DeleteMem(this->wom_);
    }
...
void DPFORAM::BlockPIR(const unsigned long addr_23[2],
                       ...
                       uchar *fss_out[2]) {
    uchar *keys[2];
    uint keyBytes = fss_.Gen(addr_23[0] ^ addr_23[1], this->log_n_, keys);
    this->cons_[0]->Write(keys[0], keyBytes);
    this->cons_[1]->Write(keys[1], keyBytes);
    this->cons_[0]->Read(keys[1], keyBytes);
    this->cons_[1]->Read(keys[0], keyBytes);
...
#pragma omp critical
    ...
    cons_[0]->Write(block_23[0], data_bytes_);
    cons_[1]->Read(block_23[1], data_bytes_);
...
void DPFORAM::AppendCache(const uchar *const block_23[2],
    ...
    for (uint i = 0; i < 2; i++) {
        cal_xor(block_23[i], delta_block_23[i], data_bytes_, cache_[i][cache_ctr_]);
    }
    cache_ctr_++;
    if (cache_ctr_ == n_) {
        InitCacheCtr();
...
void DPFORAM::WOM2ROM() {
    ...
    for (unsigned long i = 0; i < n_; i++) {
        memcpy(rom_[0][i], wom_[i], data_bytes_);
    }
    for (unsigned long i = 0; i < n_; i++) {
        cons_[0]->Write(wom_[i], data_bytes_);
        //		cons[0]->fwrite(wom[i], DBytes);
    }
    //	cons[0]->Flush();
    for (unsigned long i = 0; i < n_; i++) {
        cons_[1]->Read(rom_[1][i], data_bytes_);
...
void DPFORAM::Access(const unsigned long addr_23[2], const uchar *const new_rec_23[2],
    ...
    long_to_bytes(this->cache_ctr_, new_stash_ptr, this->log_n_bytes_);
    new_stash_ptr[0] = 1;
    ...
    BlockPIR(stash_addrPre_23, this->cache_, this->cache_ctr_, stash_block_23,
             stash_fss_out);
