...
void SSOT::P2(const uint data_size) {
    ...
    this->conn_[P0]->Write(y, data_size);
    // Send y0, y1, x, beta to P1
    this->conn_[P1]->Write(x, data_size);
}

void SSOT::P0(const uint b0, const uchar *u01[2], const uint data_size, uchar *p0) {
    ...
    uint alpha = this->prgs_[P1].GenerateBit();

    // Send s to P1
    uint s = b0 ^ alpha;
    this->conn_[P1]->WriteInt(s);
    ...
    this->conn_[P1]->Write(u01_p[0], data_size);
    this->conn_[P1]->Write(u01_p[1], data_size);
...
void SSOT::P1(const uint b1, const uchar *v01[2], const uint data_size, uchar *p1) {
    ...
    uint beta = this->prgs_[P0].GenerateBit();

    // Send t to P0
    uint t = b1 ^ beta;
    this->conn_[P0]->WriteInt(t);
    ...
    this->conn_[P0]->Write(v01_p[0], data_size);
    this->conn_[P0]->Write(v01_p[1], data_size);
