...
public:
    FSS1Bit();
    std::pair<std::vector<BinaryData>, bool> Gen(const uint64_t index, const uint64_t log_n, const bool is_symmetric);
    bool Eval(BinaryData &query, const uint64_t index);
    bool *EvalAll(BinaryData &query_23, const uint64_t log_n);
